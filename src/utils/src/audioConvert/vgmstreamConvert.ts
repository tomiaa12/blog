 ...
 * @example 基础用法
 * ```ts
 * import { vgmstreamConvert } from '@/utils'
 *
 * const results = await vgmstreamConvert([file1, file2, file3])
 * results.forEach(r => {
 ...
 * import { vgmstreamConvert } from '@/utils'
 *
 * const results = await vgmstreamConvert(files, (file, progress) => {
...
/** 单个文件的转换结果 */
export interface VgmstreamConvertResult {
  /** 原始 WEM 文件对象 */
  file: File
 ...
 */
export type VgmstreamProgressCallback = (file: File, progress: number) => void

// ─── 内部单例 Worker ──────────────────────────────────────────────────────────

const WORKER_URL = '/vgmstream/cli-worker.js'
 ...
 * ```ts
 * // 基础
 * const results = await vgmstreamConvert([file1, file2])
 *
 * // 带进度
 * const results = await vgmstreamConvert(files, (file, progress) => {
 *   console.log(file.name, progress + '%')
 ...
 * ```
 */
export async function vgmstreamConvert(
  files: File[],
  onProgress?: VgmstreamProgressCallback,
): Promise<VgmstreamConvertResult[]> {
  if (!files.length) return []

  if (typeof window === 'undefined') {
    throw new Error('vgmstreamConvert 只能在浏览器环境中使用')
  }
  ...
  const results: VgmstreamConvertResult[] = []
  ...
  for (const file of files) {
    ...
    try {
      ...
      clearInterval(timer)
      console.error(`[vgmstreamConvert] 转换失败: ${file.name}`, err)
    }
  }
