 ...
 * 配置
 */
interface CanvasGraffitiOptions {
  // canvas
  el: string | HTMLCanvasElement

  // 当前绘图工具
  currentTool?: CanvasGraffitiToolType

  // 创建离屏渲染画布样式
  createBufferCanvasStyle?: { [prop: string]: any }

  // 允许书写的方式
  allowType?: CanvasGraffitiAllowType[]
...
// 事件名
type CanvasGraffitiEventTypes = "pointerdown" | "pointermove" | "pointerup"
// 工具名
export type CanvasGraffitiToolType =
  | "Marker"
  | "Line"
  | "Rect"
  | "FillRect"
...
export type CanvasGraffitiAllowType = "mouse" | "touch" | "pen" | (string & {})
...
export class CanvasGraffiti implements ToolOptions {
  ...
  // 工具名
  currentTool: CanvasGraffitiToolType = "Marker"
  ...
  constructor(options: CanvasGraffitiOptions) {
  ...
  // 解绑事件
  removeEventListener(
    types: CanvasGraffitiEventTypes[] = [
      "pointerdown",
      "pointermove",
      "pointerup",
    ]
