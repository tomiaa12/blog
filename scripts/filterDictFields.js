...
const inputFile = resolveFromArgs(2, path.resolve(__dirname, '../src/public/dicts/merged-unique-words.json'));
const outputFile = resolveFromArgs(3, path.resolve(path.dirname(inputFile), 'merged-words-basic.json'));
...
function pickFields(entry) {
  ...
  const result = {};
  const fields = ['word', 'phonetic0', 'phonetic1', 'trans'];
